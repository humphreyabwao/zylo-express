...
export default async function JournalIndexPage() {
  /**
   * `lead` is undefined when nothing is published. That is a real state, not an
   * error: the catalogue helpers only fall back to seed data when a query
   * *fails*, so a table that exists and legitimately returns no rows — every
   * article unpublished, or RLS filtering them all out for an anonymous reader
   * — arrives here as an empty array. Destructuring it and rendering blind
   * fails the whole prerender with `Cannot read properties of undefined`.
   */
    ...
    <>
      ...
      {lead ? (
        <>
          ...
          {/* The rest. Suppressed entirely at one article, so the hairline rule
              never sits above an empty grid. */}
          {rest.length > 0 && (
        ...
        <section className="container-shell pb-section">
          <p className="border-t border-hairline pt-14 text-sm font-light leading-relaxed text-muted-foreground">
